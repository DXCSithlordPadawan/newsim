...
// JAS 39 Gripen weapon system
// Weapon loadout from gripen_config.json:
//   internal_gun: "Mauser BK-27" (27mm, 120 rounds)
//   supported_missiles_aa: ["AIM-9 Sidewinder", "IRIS-T", "AIM-120 AMRAAM"]
//   Countermeasures: BOZ-101 chaff/flare dispenser pod
export class WeaponSystem {
	constructor(viewer, scene, playerModel) {
		...
		// Gripen weapons: Mauser BK-27 cannon and IRIS-T AAM
		// gun_rounds: 120 (from gripen_config.json)
		this.weapons = [
			{ id: 'gun', name: 'MAUSER BK-27', ammo: Infinity, maxAmmo: Infinity, fireRate: 0.04, lastFire: 0 },
			{ id: 'missile', name: 'IRIS-T', ammo: 4, maxAmmo: 4, fireRate: 1.0, lastFire: 0, type: 'IRIS-T' }
		];

		// BOZ-101 chaff/flare dispenser
		this.flareWeapon = { id: 'flare', name: 'BOZ-101', ammo: 40, maxAmmo: 40, fireRate: 0.2, lastFire: 0 };
		...
		// IRIS-T has short lock time due to all-aspect seeker
		this.lockTime = 0;
	...
	fire(playerState, specificWeaponId = null) {
		...
		if (weapon.id === 'gun') {
			// Mauser BK-27: 27mm autocannon, 1700 rounds/min
			this.gunHeat += 0.025;
			if (this.gunHeat >= 1.0) {
			...
			// IRIS-T: wingtip-mounted IR missiles
			this.lastMissileSide = !this.lastMissileSide;
			const side = this.lastMissileSide ? 1 : -1;
			// Gripen wingtip missile positions
			const missileOffset = new THREE.Vector3(18.0 * side, -12.0, 0.0);
